...
typedef void (*_tWMProcess)(void);
class cCriticalSection_WM: public cCriticalSection_ {
private:
public:
    cCriticalSection_WM(_tWMProcess WMProcess = 0):_WMProcess(WMProcess) {}
    void lock(void);
protected:
    _tWMProcess _WMProcess;
...
private:
public:
    cCriticalSection_WM_cond(int forThread, _tWMProcess WMProcess = 0):cCriticalSection_WM(WMProcess), forThread(forThread) {}
    void lock(void);
protected:
	int forThread;
};
